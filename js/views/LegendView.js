define([
    ...
    var LegendView = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            $(document.body).on("hide.bs.modal", "#base-modal-legend", this, function () {
        ...
        getVisibleLayer: function () {
            var layers = this.getAllVisibleLayer(),
                legendParams = [],
                groupArray = [],
                layeridarray = [];
console.log(layers);
            _.each(layers, function (element) {
                ...
                if (element.get("typ") === "WFS") {
                    ...
                    groupArray = [];
                    _.each(element.get("layer").getLayers().getArray(), function (layerarray) {
                        layeridarray.push(layerarray.id);
                        // die Abfrage muss irgendwie sein verstehe nicht so ganz warum SD
                        if (layerarray.getSource().getUrls) {
                            groupArray.push({
                                typ: "GROUP",
...
console.log(legendParams);
        ...
        aftercollapse: function (legendObject) {
            ...
            _.each($(".legenddiv"), function (element) {
