...
describe("getqueryParams", function () {
    ...
    it("should be an empty object by iframe input", function () {
        const query = "zoomtogeometry=<iframe%20src%3dhttps://example.com%20/>";

        expect(getQueryParams(query)).to.be.an("object").that.is.empty;
    });
    it("should be an empty object by iframe with content input", function () {
    ...
    it("should be an object with key and value by zoomtogeometry string without html input", function () {
        const query = "zoomtogeometry=bergedorf";

        expect(getQueryParams(query)).to.be.an("object").to.deep.equal({zoomtogeometry: "bergedorf"});
    });
    it("should be an object with keys and values by a large string without html input", function () {
