const webdriver = require("selenium-webdriver"),
    ...
    {isDefault, is2D, isMobile} = require("../../../settings"),
...
function Button3DTests ({builder, url, resolution, mode}) {
    /* only run tests in
     * - in non-mobile mode (does not have 3D mode)
     * - in custom and default (have 3d mode)
     * - in 2D mode because de-/activating 3D is part of this test (3D mode is for rerunning tools/search tests in 3D)
     */
    const skipAll = !is2D(mode) || isMobile(resolution) || !isDefault(url);

    (skipAll ? describe.skip : describe)("Modules Controls Button3D", function () {
        ...
        before(async function () {
            ...
            // activate a layer for later testing whether it remains active
        ...
        it("3D mode tools are restricted to those worked in 3D mode", async function () {
            ...
            await driver.wait(until.elementIsNotVisible(await driver.findElement(By.xpath("//ul[@id='tools']//a[contains(.,'Einwohneranzahl abfragen')]"))));
        ...
        describe("clicking 3D button again deactivates 3D mode", function () {
            ...
            it("3D tools no longer available; 2D-only tools available again", async function () {
                await (await driver.findElement(By.xpath("//span[contains(.,'Werkzeuge')]"))).click();
                await driver.wait(until.elementIsVisible(await driver.findElement(By.xpath("//ul[@id='tools']//a[contains(.,'Einwohneranzahl abfragen')]"))));
