define(function (require) {
    ...
    describe("core/modelList/layer/group", function () {
        ...
        describe("groupGfiAttributes", function () {
            ...
            it("should return an object with gfiAttributes of groupLayer (Array)", function () {
                expect(groupLayer.groupGfiAttributes({"test1": "TEST1", "test2": "TEST2"}, layerdefinitions)).to.deep.include({test1: "TEST1", test2: "TEST2"});
            });
            it("should return a string with gfiAttribute of groupLayer", function () {
                expect(groupLayer.groupGfiAttributes(undefined, [layerdefinitions[0]])).to.equal("ignore");
            });
        });
        describe("groupLayerObjectsByUrl", function () {
            ...
            it("should return 2 layerdefinitions when gfiAttributes is set on groupLayer", function () {
                groupLayer.set("gfiAttributes", "showAll");
                expect(groupLayer.groupLayerObjectsByUrl(layerdefinitions)).to.be.an("array").that.has.lengthOf(2);
            });
            it("should return layerdefinitions as array with the same gfiAttribute when gfiAttributes is set on groupLayer", function () {
                groupLayer.set("gfiAttributes", "showAll");
                expect(groupLayer.groupLayerObjectsByUrl(layerdefinitions)).to.be.an("array").to.deep.include({
                    "id": "2802",
                    "name": "Jahrgang 2015",
                    "url": "https://geodienste.hamburg.de/HH_WMS_Historische_Karte_1_5000",
                    "typ": "WMS",
