...
describe("check Elastic Search Helper Functions", function () {
    ...
    it("prepareSearchBody should return stringyfied JSON", function () {
        expect(ElasticSearch.prepareSearchBody({query: "meine Anfrage"}, undefined, 10000)).to.be.equal("{\"from\":0,\"size\":10000,\"query\":{\"query\":\"meine Anfrage\"}}");
    });
    it("prepareSearchBody should have sort order", function () {
        model.setSorting("_score", "desc");
        expect(ElasticSearch.prepareSearchBody({query: "meine Anfrage"}, model.get("sorting"), 10000)).to.be.equal("{\"sort\":{\"_score\":\"desc\"},\"from\":0,\"size\":10000,\"query\":{\"query\":\"meine Anfrage\"}}");
    });
    ...
    it("setSize should change size to 500", function () {
        model.setSize(500);
        expect(ElasticSearch.prepareSearchBody({query: "meine Anfrage"}, {"_score": "desc"}, model.get("size"))).to.be.equal("{\"sort\":{\"_score\":\"desc\"},\"from\":0,\"size\":500,\"query\":{\"query\":\"meine Anfrage\"}}");
    });
});
