    ...
    describe("modules/searchbar", function () {
        ...
        describe("shortenNames", function () {
            it("should return name if name is shorter then length", function () {
                expect(model.shortenNames("test", 5)).to.be.an("string").that.equals("test");
            });
            it("should return name if name length is euqal to length", function () {
                expect(model.shortenNames("test", 4)).to.be.an("string").that.equals("test");
            });
            it("should return name croped to length extended with '...' if name is longer then length", function () {
                expect(model.shortenNames("test", 3)).to.be.an("string").that.equals("tes...");
            });
            it("should not throw error if name is undefined", function () {
                expect(model.shortenNames(undefined, 1)).to.be.undefined;
            });
        });
    });
});
