<?php
	...
	# Prüfe Eingabedaten
	if ($orderid === "") {
		error_log("cancel.php is missing ORDERID. Aborting!");
		exit();
	}
	...
	# Logge Status
	$query = "UPDATE " . $pg_table . " SET status = 'canceled' WHERE orderid = '" . $orderId . "' RETURNING *;";
	write2DB($query);

	# Erzeuge returnURL für redirect
	$returnURL = $portalURL . "?STATUS=CANCEL&ORDERID=" . $orderId;
