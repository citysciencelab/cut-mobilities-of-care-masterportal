...
|isInputMap|nein|Boolean|false||Flag, dass die Eingabe von Koordinaten über das Masterportal per Mausklick erleubt. Wenn TRUE, wird bei jedem Klick in die Karte automatisch ein Marker auf der Karte gesetzt und die Koordinaten des Markers per RemoteInterface als Objekt gesendet.|
|targetProjection|nein|String| |_Nur wirksam bei isInputMap = true!_ Zielprojektion der Koordninaten, die das Portal bei einem Klick in das RemoteInterface sendet.|
