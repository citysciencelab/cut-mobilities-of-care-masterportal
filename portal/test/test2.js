    ...
    describe("downloadModel", function () {
        describe("validateFilename", function () {
            ...
            it("validation should fail when filename ends with \".\"", function () {
                assert.isFalse(Model.validateFilename("name."), "filenames beginning with '.' should not be valid'");
            });
            it("validation should fail when filename contains \"?\"", function () {
                assert.isFalse(Model.validateFilename("nam?e.jpg"), "filenames beginning with '.' should not be valid'");
            });
            it("validation should fail when filename isEmpty", function () {
                assert.isFalse(Model.validateFilename(""), "filenames beginning with '.' should not be valid'");
            });
